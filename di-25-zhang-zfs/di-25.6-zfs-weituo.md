...
此处，`zroot/home/safreya/secret` 继承了 `zroot/home/safreya` 的权限属性，因此授权或取消授权操作仅针对 `zroot/home/safreya`，对 `zroot/home/safreya/secret` 的权限不会生效。因为操作后属性保持不变，因为 ZFS 不支持“动态继承刷新”：子数据集只会在“创建时”继承父数据集的委托权限，之后父数据集的授权或反授权变更，不会自动向下同步到已存在的子数据集。这种授权是“静态继承设计”，目的是防止父级权限变更意外破坏子数据集的既有管理策略，需要显式、逐级授权或回收。
